...
extension DefaultEmbyAPIClient {
    func fetchInstantMixImpl(fromItemId: String, type: InstantMixKind) async throws -> [JellyfinTrackDTO] {
        let path: String
        switch type {
        case .album:
        ...
        var components = URLComponents(url: buildURL(path: path), resolvingAgainstBaseURL: false)!
